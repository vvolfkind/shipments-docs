...
**Authority:** Este documento es la fuente de verdad para definiciones de dominio. Si `plan.md`, `logistics.md` o el código contradicen este archivo, **este archivo gana** hasta que se actualicen explícitamente.
...
**Regla cerrada:** todo aviso OOLL reconciliado que cambie `canonicalStatus`, `subStatus` y/o timestamp operativo **debe** emitir `PACKAGE_NEW_MILESTONE` al projector — **solo si hay MATCH** de package en API (MATCH/UNMATCHED policy: `plan.md` § B.5). Sin MATCH: persistir inbox `UNMATCHED`; sin emisión (v1 forward-only).
...
| Tema | Documento |
|------|-----------|
| Plan operativo + status + reglas agente | **`plan.md`** (única fuente) |
| E2E local + runbook + SQL validación | `testing-orchestrator/README.md` |
| Andreani prod wire samples | `andreaniEvents.md` |
| Fenix bulk relay | `plan.md` § 6 (B.7) |
| Terminales última milla | `plan.md` § 6 (B.8) · §2.1 |
| Diseño por block (API/projector) | implementado en código + tests; decisiones en `plan.md` |
| Flujo negocio / físico | `logistics.md` |
